...
impl Command {
    pub fn is_applicable(&self, state: &BattleState) -> bool {
        match *self {
            Self::Defend => is_applicable_defend(),
            Self::Wait => is_applicable_wait(state),
            Self::Move { destination } => is_applicable_move(state, destination),
            Self::Attack {
                attack_position,
                attack_direction,
            } => is_applicable_attack(state, attack_position, attack_direction),
            Self::Shoot { target } => is_applicable_shoot(state, target),
        }
    }
    pub fn apply(&self, state: &mut BattleState) {
        match *self {
            Self::Defend => apply_defend(state),
            Self::Wait => apply_wait(state),
            Self::Move { destination } => apply_move(state, destination),
            Self::Attack {
                attack_position,
                attack_direction,
            } => apply_attack(state, attack_position, attack_direction),
            Self::Shoot { target } => apply_shoot(state, target),
        }
